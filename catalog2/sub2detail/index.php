...
$s2=dunderscore($_GET['s2']);
$query = "SELECT Type_of_application FROM products_june WHERE product_type='$s2' AND Product_Group='$s1' AND System='$pg' GROUP BY Type_of_application ORDER BY Type_of_application ASC";		
$result = mysql_query ($query); // Run the query.
if (!$result) { echo $query;}
if (mysql_num_rows($result) > 0) {
$s1array = array();
while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
$aparray[] = $row['Type_of_application'];
}
}
...
<span class="breadcrumbs"><a href="/index.php"><nobr>Home</nobr></a> > <nobr><a href="/catalog2/default.php">Catalog</a></nobr> > <nobr><?PHP echo dunderscore($_GET['c']);?></nobr></span>
   <br />
&nbsp;<br />
 <h1><?PHP echo dunderscore($_GET['c']).': '.dunderscore($_GET['s1']).': '.dunderscore($_GET['s2']);?></h1>
<span class="content">
...
$query = "SELECT * FROM products_june WHERE Product_Group='$s1' AND System='$pg' and product_type='$s2' ORDER BY Photo DESC, Manufacturer ASC ";		
$result = mysql_query ($query); // Run the query.
//echo $query;
echo '<h2>Products</h2>';
...
while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
if ($row['Master_Item_Part_No'] !='') {$in = $row['Master_Item_Part_No'];

if ($row['Photo'] != '') {$thisimage = imagename($row['Photo']);
...
class="inner"><div id="rightcontent"><span class="rightcontent"><div class="cmxvspace" style="height:28px"></div><div style="clear:both"></div> <h2 style="padding-top:5px;">Search:</h2><span><form name="frmSearchBox" action="/ssearch/search.php" method="post">
...
<input type="hidden" value="search.php" name="result_page"/> 
 ...
 /><input type="submit" value="Go"/><br />&nbsp;
   </form></span><div class="cmxvspace" style="height:26px"></div><div style="clear:both"></div> <span class="calls-to-action"><ul>
